...
fn make_file_entry(
    ...
    FileEntry {
        ...
        functions: (0..functions_count)
            .map(|i| codegraph::graph::FunctionInfo {
                name: format!("fn{}", i),
            ...
            .collect(),
        classes: (0..classes_count)
            .map(|i| codegraph::graph::ClassInfo {
                name: format!("Class{}", i),
                start_line: 1,
                end_line: 5,
            })
...
fn make_graph() -> CodeGraph {
    ...
    files.insert(
        ...
        make_file_entry("auth", "sha256:aaa", 1, 0),
    );
    files.insert(
        "src/api/routes.ts".to_string(),
        make_file_entry("api", "sha256:bbb", 1, 0),
    );
    files.insert(
        "src/old/removed.ts".to_string(),
        make_file_entry("old", "sha256:ccc", 0, 0),
    );
...
fn test_merge_removes_file() {
    ...
    assert!(
        graph.files.get("src/old/removed.ts").is_none(),
...
fn test_merge_removes_empty_module() {
    ...
    assert!(
        graph.modules.get("old").is_none(),
        "empty module should be removed"
    );
}
...
fn test_merge_updates_file() {
    ...
    updated.insert(
        "src/api/routes.ts".to_string(),
        make_file_entry("api", "sha256:xxx", 2, 0),
    );
...
fn test_merge_adds_new_file_and_module() {
    ...
    let mut updated = HashMap::new();
    updated.insert(
        "src/new/added.ts".to_string(),
        make_file_entry("newmod", "sha256:ddd", 1, 1),
    );
    merge_graph_update(&mut graph, updated, &[]);
    assert!(
        graph.files.get("src/new/added.ts").is_some(),
        "added file should exist"
    );
    assert!(
        graph.modules.get("newmod").is_some(),
        "new module should exist"
    );
    assert!(graph.modules["newmod"]
        .files
        .contains(&"src/new/added.ts".to_string()));
}
